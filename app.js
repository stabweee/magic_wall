...
async function init(year) {
  try {
    ...
    let hasData = false;
    if (year === 2020) preloadData = preloadData2020;
    else if (year === 2024) preloadData = preloadData2024;
...
function setupEventListeners() {
  ...
  yearMenu.addEventListener("change", yearMenuHandler);
...
function renderStates() {
  ...
  const allCountiesCount = topojson.feature(
    usData,
    usData.objects.counties,
  ).features;
  ...
  stateGroups
    ...
    .on("mouseover", (event, d) => {
      const stats = getStateStats(d.id, allCountiesCount);
      const info =
        stats.totalColored > 0
          ? `Majority: ${Math.round(stats.majorityPercentage)}%`
          : "";
    ...
    if (stats.totalColored > 0) {
      ...
      if (!isNaN(centroid[0])) {
        ...
        group
          ...
          .attr("x", centroid[0])
          .attr("y", centroid[1] + 5)
          .text(`${Math.round(stats.majorityPercentage)}%`);
...
function renderCounties(stateFeature) {
  ...
  viewLabel.innerText = `${stateFeature.properties.name} • Drag to pan • Scroll to zoom`;
  ...
  g.selectAll(".county-path")
    ...
    .on("mouseover", (event, d) => {
      const data = filledColors.get(`county-${d.id}`);
      const info =
        data.gop_votes > data.dem_votes
          ? `Majority: ${(data.gop_per * 100).toFixed(2)}%`
          : `Majority: ${(data.dem_per * 100).toFixed(2)}%`;
      showTooltip(event, d.properties.name, info);
    })
    .on("mouseout", hideTooltip)
    .on("click", (event, d) => {
...
function zoomToFeature(feature) {
  ...
  const scale = Math.max(
    1,
    Math.min(10, padding / Math.max(dx / width, dy / height)),
  );
